# Machine Learning Online Courses
The following is the content of this page 
- [Description](https://github.com/SSQ/Machine-Learning-Online-Courses/blob/master/README.md#description)
- [Content](https://github.com/SSQ/Machine-Learning-Online-Courses/blob/master/README.md#content)
- [Detail](https://github.com/SSQ/Machine-Learning-Online-Courses/blob/master/README.md#detail)
## Description
- The following is a list of online courses about machine learning. (enrolled, completed, interested)
- The order does not matter
- The list is created on my own experience
- I will keep updating this repo.

## Content
- Stanford [Machine Learning | Coursera](https://www.coursera.org/learn/machine-learning)  
- UW [Machine Learning | Coursera](https://www.coursera.org/specializations/machine-learning) 
    - [Machine Learning Foundations: A Case Study Approach | Coursera](https://www.coursera.org/learn/ml-foundations)
    - [Machine Learning: Regression | Coursera](https://www.coursera.org/learn/ml-regression)
    - [Machine Learning: Classification | Coursera](https://www.coursera.org/learn/ml-classification)
    - [Machine Learning: Clustering & Retrieval | Coursera](https://www.coursera.org/learn/ml-clustering-and-retrieval)
- Stanford CS229 [Machine Learning](https://cs229.stanford.edu/)
- Stanford [Algorithms | Coursera](https://www.coursera.org/specializations/algorithms)
    - [Divide and Conquer, Sorting and Searching, and Randomized Algorithms | Coursera](https://www.coursera.org/learn/algorithms-divide-conquer)
    - [Graph Search, Shortest Paths, and Data Structures | Coursera](https://www.coursera.org/learn/algorithms-graphs-data-structures)
    - [Greedy Algorithms, Minimum Spanning Trees, and Dynamic Programming | Coursera](https://www.coursera.org/learn/algorithms-greedy)
- UIUC [Data Mining  | Coursera](https://www.coursera.org/specializations/data-mining) 
    - [Data Visualization | Coursera](https://www.coursera.org/learn/datavisualization)
    - [Text Retrieval and Search Engines | Coursera](https://www.coursera.org/learn/text-retrieval)
- Ng [Deep Learning | Coursera](https://www.coursera.org/specializations/deep-learning)
    - [Neural Networks and Deep Learning | Coursera](https://www.coursera.org/learn/neural-networks-deep-learning)
    - [Improving Deep Neural Networks: Hyperparameter tuning, Regularization and Optimization | Coursera](https://www.coursera.org/learn/deep-neural-network)
    - [Structuring Machine Learning Projects | Coursera](https://www.coursera.org/learn/machine-learning-projects)
    - [Convolutional Neural Networks | Coursera](https://www.coursera.org/learn/convolutional-neural-networks)
- Stanford [CS231n: Convolutional Neural Networks for Visual Recognition](https://cs231n.stanford.edu/)
- Rice [Fundamentals of Computing | Coursera](https://www.coursera.org/specializations/computer-fundamentals)
    - [An Introduction to Interactive Programming in Python (Part 1) | Coursera](https://www.coursera.org/learn/interactive-python-1)
    - [An Introduction to Interactive Programming in Python (Part 2) | Coursera](https://www.coursera.org/learn/interactive-python-2)
    - [Principles of Computing (Part 1) | Coursera](https://www.coursera.org/learn/principles-of-computing-1)
- Peking [程序设计与算法 | Coursera](https://www.coursera.org/specializations/biancheng-suanfa)
    - [计算导论与C语言基础 | Coursera](https://www.coursera.org/learn/jisuanji-biancheng)
    - [C程序设计进阶 | Coursera](https://www.coursera.org/learn/c-chengxu-sheji)
- HSE [Advanced Machine Learning Specialization | Coursera](https://www.coursera.org/specializations/aml)
    - [Introduction to Deep Learning | Coursera](https://www.coursera.org/learn/intro-to-deep-learning)
- [Machine Learning Engineer Nanodegree | Udacity](https://www.udacity.com/course/machine-learning-engineer-nanodegree--nd009)
- [Deep learning | Udacity](https://www.udacity.com/course/deep-learning--ud730)
- [How to Use Git and GitHub | Udacity](https://www.udacity.com/course/how-to-use-git-and-github--ud775)
- [Intro to Computer Science | Udacity](https://www.udacity.com/course/intro-to-computer-science--cs101)
- [Programming Foundations with Python | Udacity](https://www.udacity.com/course/programming-foundations-with-python--ud036)


## Detail
### 1. Stanford [Machine Learning | Coursera](https://www.coursera.org/learn/machine-learning) 
- **Recommend**, fundamental concepts

- **Time consuming:** two weeks, or one week in hard-working

- **Schedule:** Completed

- **Note:** None

- **I want to say:** This is my first course in machine learning.



### 2. University of Washington [Machine Learning | Coursera](https://www.coursera.org/specializations/machine-learning) 
- **Courses:**
  1. [x] [Machine Learning Foundations: A Case Study Approach | Coursera](https://www.coursera.org/learn/ml-foundations)
  1. [x] [Machine Learning: Regression | Coursera](https://www.coursera.org/learn/ml-regression)
  1. [x] [Machine Learning: Classification | Coursera](https://www.coursera.org/learn/ml-classification)
  1. [x] [Machine Learning: Clustering & Retrieval | Coursera](https://www.coursera.org/learn/ml-clustering-and-retrieval)

- **Highly Recommend,** machine learning algorithms

- **Time consuming:** one day per week, from 9:00am to 24:00pm, I'm using pandas, it could be faster for sframe.

- **Schedule:** 4/4, Completed

- **Note:** None

- **I want to say:** 
  - This course teach you how to build a model from scratch and how to use it. 
  - Programming assignments are very time consuming.


### 3. Stanford CS229 [Machine Learning](https://cs229.stanford.edu/)
- **Recommend,** machine learning algorithms

- **Time consuming:** Very slow for personal reason.

- **Schedule:** Interested

- **Note:** None

- **I want to say:** None



### 4. Stanford [Algorithms | Coursera](https://www.coursera.org/specializations/algorithms) 

- **Courses:**
  1. [x] [Divide and Conquer, Sorting and Searching, and Randomized Algorithms | Coursera](https://www.coursera.org/learn/algorithms-divide-conquer)
  1. [x] [Graph Search, Shortest Paths, and Data Structures | Coursera](https://www.coursera.org/learn/algorithms-graphs-data-structures)
  1. [x] [Greedy Algorithms, Minimum Spanning Trees, and Dynamic Programming | Coursera](https://www.coursera.org/learn/algorithms-greedy)

- **Recommend,** computer science algorithms

- **Time consuming:** one day per week, from 9:00am to 24:00pm

- **Schedule:** 3/4 Completed

- **Note:** None

- **I want to say:** 
  - I have completed the first three courses. Many programming assignments need more optimization. 
  - Programming assignments are very time consuming. 
  - Learn how to think as a computer scientist.



### 5. UIUC [Data Mining  | Coursera](https://www.coursera.org/specializations/data-mining) 

- **Courses:**
  1. [x] [Data Visualization | Coursera](https://www.coursera.org/learn/datavisualization)
  1. [x] [Text Retrieval and Search Engines | Coursera](https://www.coursera.org/learn/text-retrieval)

- **Recommend,** data mining

- **Time consuming:** one day per week, from 9:00am to 24:00pm

- **Schedule:** 2/6 Completed

- **Note:** None

- **I want to say:** 
  - I have completed the only first two courses for two reasons. Time consuming and problems about programming assignments. 
  - The first course is helpful to those want to be a data analysis. 
  - The second course introduce the fundamental concepts and application.



### 6. Ng [Deep Learning | Coursera](https://www.coursera.org/specializations/deep-learning)
- **Courses:**
  1. [x] [Neural Networks and Deep Learning | Coursera](https://www.coursera.org/learn/neural-networks-deep-learning)
  1. [x] [Improving Deep Neural Networks: Hyperparameter tuning, Regularization and Optimization | Coursera](https://www.coursera.org/learn/deep-neural-network)
  1. [x] [Structuring Machine Learning Projects | Coursera](https://www.coursera.org/learn/machine-learning-projects)
  1. [ ] [Convolutional Neural Networks | Coursera](https://www.coursera.org/learn/convolutional-neural-networks)
  
- **Highly Recommend,** deep learning

- **Time consuming:** one day or a half per week

- **Schedule:** 4/5

- **Note:** None

- **I want to say:** I'm taking. I love this specialization




### 7. Stanford [CS231n: Convolutional Neural Networks for Visual Recognition](https://cs231n.stanford.edu/)

- **Recommend,** deep learning

- **Time consuming:** Very slow for personal reason.

- **Schedule:** interested

- **Note:** None

- **I want to say:** I only complete the CNN part of this course




### 8. Rice University [Fundamentals of Computing | Coursera](https://www.coursera.org/specializations/computer-fundamentals)
- **Courses:**
  1. [x] [An Introduction to Interactive Programming in Python (Part 1) | Coursera](https://www.coursera.org/learn/interactive-python-1)
  1. [x] [An Introduction to Interactive Programming in Python (Part 2) | Coursera](https://www.coursera.org/learn/interactive-python-2)
  1. [ ] [Principles of Computing (Part 1) | Coursera](https://www.coursera.org/learn/principles-of-computing-1)

- **Recommend,** I want to proficient in Python

- **Time consuming:** one evening or two

- **Schedule:** 3/7 

- **Note:** None

- **I want to say:** I'm taking. Very interesting tiny games. Making progress for proficient in Python






### 9. Peking University [程序设计与算法 | Coursera](https://www.coursera.org/specializations/biancheng-suanfa)
- **Courses:**
  1. [x] [计算导论与C语言基础 | Coursera](https://www.coursera.org/learn/jisuanji-biancheng)
  1. [ ] [C程序设计进阶 | Coursera](https://www.coursera.org/learn/c-chengxu-sheji)
  
- **Recommend,** I want to proficient in C/C++

- **Time consuming:** two evenings

- **Schedule:** 2/7 

- **Note:** None

- **I want to say:** I'm taking. Making progress for familiar with C/C++





### 10. HSE [Advanced Machine Learning Specialization | Coursera](https://www.coursera.org/specializations/aml)
- **Courses:**
  1. [ ] [Introduction to Deep Learning | Coursera](https://www.coursera.org/learn/intro-to-deep-learning)

- **Recommend,** machine learning algorithms

- **Time consuming:** one day per week

- **Schedule:** 1/6

- **Note:** None

- **I want to say:** I'm taking




### 11. [Machine Learning Engineer Nanodegree | Udacity](https://www.udacity.com/course/machine-learning-engineer-nanodegree--nd009)
- **Courses:**

- **Recommend,**

- **Time consuming:** one month

- **Schedule:** Capstone

- **Note:** None

- **I want to say:** 
  - Add projects in your resume. 
  - Teach you the application of machine learning. 
  - Consisting of supervised learning, unsupervised learning, reinforcement learning and deep learning. 
  - It is not too difficult to complete the projects, but plenty of skills need to know.



### 12. [Deep learning | Udacity](https://www.udacity.com/course/deep-learning--ud730)

- **Not Recommend for fresh machine learning,** deep learning

- **Time consuming:** 

- **Schedule:** Completed

- **Note:** None

- **I want to say:** 
  - It is a little difficult for those fresh machine learners.
  - Not as good as deep learning part of [machine learning nano degree](https://www.udacity.com/course/machine-learning-engineer-nanodegree--nd009)




### 13. [How to Use Git and GitHub | Udacity](https://www.udacity.com/course/how-to-use-git-and-github--ud775)

- **Recommend,** Git and Github

- **Time consuming:** eithor one day or three days, I forget

- **Schedule:** Completed

- **Note:** None

- **I want to say:** Good course for me, but I do not use all the skills learned from this coures.




### 14. [Intro to Computer Science | Udacity](https://www.udacity.com/course/intro-to-computer-science--cs101)

- **Recommend,** I want to know more about Python

- **Time consuming:** Very long, 2 months

- **Schedule:** Completed

- **Note:** 
  - Course's description, content, question and answer in Udacity can be found in [my blog](https://ssq.github.io/2017/02/05/Udacity%20cs101%20Intro%20to%20CS%20Notebook/)

- **I want to say:** 
  - time consuming
  - Introduction to computer science via python and web crawling
  - I wrote a script to crawl the content of Udacity to write my blog after this course




### 15. [Programming Foundations with Python | Udacity](https://www.udacity.com/course/programming-foundations-with-python--ud036)

- **Highly Recommend,** short introduction to python

- **Time consuming:** one day or two days

- **Schedule:** Completed

- **Note:** None

- **I want to say:** 
  - Good course for fresh programmers. 
  - You would know function, how to use class, how to build class after this course



